app.controller('Init', function ($scope, Data, $http, $filter) {
    ...
    Data.get('load_datos_init').then(function (result) {
        ...
        $scope.productogrupo=result.productogrupo;
        $scope.clientetipo=result.clientetipo;
	...
	$scope.buscar = function(){
		...
		formData =  { 
	                    ...
	                    "cliente" : $scope.validate($scope.cliente_select),
                            "servicio": $scope.validate($scope.selectedTipoServicio),
                            "productogrupo": $scope.validate($scope.selectedProductogrupo),
                            "clientetipo": $scope.validate($scope.selectedClientetipo)
                	};
        $http.post('ventasjs/find',formData).then(function (result) {
            if(result){
                if($scope.selectedProductogrupo){
                    $scope.viewproductogrupo = true;
                    Data.get('productoGrupo/'+$scope.selectedProductogrupo).then(function (result) {
                        if(result.length > 0){
                            $scope.Prodogrupo = result[0]['nombre'];
                        }
                    });
                }else{
                    $scope.viewproductogrupo = false;
    ...
    $scope.load_color = function(estado,autorizado, tipo_comp, descargado){
    	var color_send;
    	if(estado < 0){
    		color_send = 'red';
    	}
    	if(estado == 1){
            if(tipo_comp==55 && descargado==1){
                color_send = 'blue';  
            }else{
                color_send = 'black';  
            }
